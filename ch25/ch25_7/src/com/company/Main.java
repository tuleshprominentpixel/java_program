...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ProminentTrainee element = prominentTraineeList.get(3);
        log.info("3nd index  trainee name: " + element.getName() + " id : " + element.getId() + " date " + element.getDob());
        ...
        printAllList(prominentTraineeList);
        prominentTraineeList.sort(Comparator.comparing(ProminentTrainee::getName));
        ...
        prominentTraineeList.sort(new DateComparator());
        printAllList(prominentTraineeList);
        ...
        try (FileWriter filewr = new FileWriter(fileName)) {
            for (int i = 0; i < (long) prominentTraineeList.size(); i++) {
        ...
        try (FileOutputStream binaryFileOutputStream = new FileOutputStream((binaryFileName))) {
            ...
            binaryObjectOutputStream.writeObject(p1);
            binaryObjectOutputStream.writeObject(p2);
            binaryObjectOutputStream.writeObject(p3);
            binaryObjectOutputStream.writeObject(p4);
            binaryObjectOutputStream.writeObject(p5);

            binaryObjectOutputStream.close();
        } catch (Exception e) {
            log.info("exception to write binary file : " + e);
        }


        try (FileInputStream fi = new FileInputStream((binaryFileName))) {
